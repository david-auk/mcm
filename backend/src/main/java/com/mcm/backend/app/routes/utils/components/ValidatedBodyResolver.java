...
/**
 * Resolver that constructs and validates a request body based on {@link ValidatedBody}.
 * It uses {@link RequestBodyUtil} for field-level validation and reflection to instantiate
 * the entity using the constructor marked with {@link TableConstructor}.
 */
@Component
public class ValidatedBodyResolver implements HandlerMethodArgumentResolver {

    // TODO Add application/json header/data check
    ...
    /**
     * Checks if the method parameter is annotated with {@link ValidatedBody}.
     *
    ...
    /**
     * Resolves the method argument by deserializing and validating the request body,
     * and constructing the target object using annotations.
     *
     * @param parameter     method parameter
     * @param mavContainer  container
     * @param webRequest    current request
     * @param binderFactory binder factory
     * @return constructed and validated object
     */
    @Override
    public Object resolveArgument(MethodParameter parameter,
        ...
        // Read JSON request body into Map
        ...
        Object[] args = Arrays.stream(tableConstructor.getParameters())
                .map(p -> {
                    try {
                        ...
                        throw new RuntimeException(e); // handled by @ControllerAdvice later
                    }
                })
                .toArray();
    ...
    /**
     * Resolves a single constructor parameter based on annotations.
     *
     * @param p         the constructor parameter
     * @param bodyUtil  request body wrapper
     * @return resolved value or null for optional primary keys
     * @throws JsonErrorResponseException if required field is missing or invalid
     */
    private Object resolveParameterValue(Parameter p, RequestBodyUtil bodyUtil, Class<?> targetClass)
        ...
        if (matchingField != null && matchingField.isAnnotationPresent(PrimaryKey.class)) {
            ...
            if (bodyUtil.containsField(name)) {
                ...
                return null; // let constructor handle default PK
            }
        }
        ...
        if (matchingField != null && matchingField.isAnnotationPresent(TableField.class)) {
            ...
            // Fallback if no annotation — assume field name == JSON key
            name = paramName;
            type = p.getType();
        }
